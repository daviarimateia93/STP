...
public class Transporter extends STPObject {
    ...
    public void receive() throws STPException {
        try {
            ...
            final Payload payload = new Payload(payloadPosition, payloadContentLength, payloadTotalLength,
                    payloadContent);
    ...
    private static void validatePayload(final Payload payload) throws STPException {
        ...
        if (payload.getLength() < 0 || payload.getLength() > PAYLOAD_MAX_SIZE
                || payload.getTotalLength() > PAYLOAD_MAX_SIZE) {
